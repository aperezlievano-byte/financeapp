...
- `src/app/api/webhooks/telegram/route.ts` — **solo normaliza y encola**: verifica, normaliza, inserta en `inbound_messages` respetando la unicidad, y llama al pipeline. Cero lógica de negocio, cero llamadas al modelo dentro del handler.
...
- El texto de la pregunta se arma con esta plantilla exacta, y el paso 10 y `tests/integration/telegram-confirm.test.ts` la comparan byte a byte:
  ...
  Responde Sí o No.
  ```
- `src/server/ingest/telegram.ts` — se edita para delegar en `pipeline.ts` y para implementar `reply`.
- `tests/integration/telegram-confirm.test.ts` — el mensaje del enunciado produce exactamente el texto `Detecté: Club de tiro, $100.000, cuenta de ahorros — ¿confirmo?\nResponde Sí o No.`; responder `sí` escribe una fila en `transactions` y cierra el estado; responder `no` deja `status='rejected'` y cero transacciones; responder `quizás` reenvía el mismo texto y deja el estado intacto.
...
**Do**
- `src/app/(app)/layout.tsx` — shell con `<header>`, `<nav>`, `<main>`, enlace de salto al contenido, y el `ThemeToggle`. Un solo `<h1>` por página.
...
- `src/app/(app)/actions.ts` — `createManual` y `softDelete` con `requireUser()` en la primera línea, validación zod y `revalidatePath("/")`.
...
| `tests/setup.ts`, `tests/e2e/global-setup.ts`, `tests/fixtures/*` | Los ejecuta la suite | — |
