...
- `tests/e2e/**` no importa nada de `src/`: verifica por la interfaz y consulta la base con `docker compose exec -T db psql`. Cuando el `psql` de un test necesita capturar el valor de un `INSERT ... RETURNING` (para reusar el id insertado en la aserción o en la interfaz), la invocación lleva **`-q` además de `-t -A`**: sin `-q`, psql añade el tag de finalización (`INSERT 0 1`) como línea extra después del valor devuelto, y el string capturado queda corrupto para cualquier uso posterior (comparación, siguiente consulta, `WHERE id = '<valor>'`). `-t -A` sola alcanza para un `SELECT` puro.
...
**Do**
- `src/server/ingest/extract-statement.ts` — manda el PDF al gateway como adjunto y pide **una lista** de movimientos, no uno. No se instala ninguna librería de PDF: el gateway acepta el documento directamente, lo que evita una dependencia más que esta sesión no pudo verificar. A diferencia de `extract-free-text.ts`, un movimiento sin fecha legible se guarda con `occurredOn` **null**, nunca "hoy" — adivinar aquí inventaría un dato que la revisión necesita corregir. Cuando `e2eBypassUserId()` (paso 3) está activo, sustituye el `AiClient` por uno fijo en vez de `defaultClient()` — ver la nota de §13 más abajo.
- `src/server/ingest/statement-batch.ts` — para cada movimiento calcula `source_ref = sha256(indice|fecha|descripcion|monto|nombreArchivo)` y escribe un pendiente por movimiento (el `indice` es la posición del movimiento dentro del extracto). Reprocesar el mismo extracto no duplica: la unicidad `(user_id, source, source_ref)` de §4 lo impide al confirmar, y el batch salta los `source_ref` ya confirmados.
...
- `tests/integration/statements.test.ts` — con gateway falso y `tests/fixtures/statement-sample.pdf`: tres movimientos producen tres pendientes; reprocesar el mismo archivo devuelve `conflict` del documento y cero pendientes nuevos; un movimiento sin fecha se guarda con `occurred_on` nulo y `status='awaiting_review'` para que la revisión lo pida. Todo contenido que un caso vaya a confirmar (y por lo tanto vaya a escribir en `transactions`) usa un `filename` único por corrida, nunca uno fijo — el `source_ref` lo incluye, y una fila confirmada en una corrida anterior sobrevive entre corridas de `pnpm test`.
- `tests/e2e/subir.spec.ts` — subir el PDF por la interfaz y ver los pendientes en `/revision`.
- **Nota sobre §13:** este es el primer paso con un flujo que toca IA y necesita cobertura e2e real (ni E1-T6/T7 ni el paso 11 la tuvieron). §13 exige que toda prueba, e2e incluida, inyecte un `AiClient` falso — pero `tests/e2e/**` no puede importar `src/` (regla de §3) para pasarle uno al servidor real que corre `pnpm test:e2e`. La solución es que el servidor mismo sustituya el cliente cuando corre bajo e2e, con el mismo triple interlock que ya usa `e2eBypassUserId()` (paso 3: `E2E_USER_ID` presente, `DATABASE_URL` termina en `_test`, y coincide con `E2E_DATABASE_URL`) — esas dos variables nunca llegan por ningún `.env`, así que esto jamás se activa contra dev o producción. Cualquier extractor futuro que necesite cobertura e2e sigue el mismo patrón.
...
**Do**
- `src/server/ingest/import-excel.ts` — lee el libro con `exceljs`, espera las columnas `fecha`, `descripcion`, `monto`, y para cada fila calcula `source_ref = sha256(fecha|descripcion|monto)`, `amount_cents = |monto| * 100` con aritmética entera y `direction = monto < 0 ? "out" : "in"`. **Este es el único camino que escribe en `transactions` sin pasar por la pantalla de revisión**, y lo hace igual a través de `commit.ts` y `withAudit`, con `action='import.excel'`: es una importación única de datos que Alejandro ya confirmó al escribirlos en su hoja. La hoja no trae columna de cuenta — `/importar` le pide al usuario que elija la cuenta destino una vez, y todo el archivo entra ahí. `exceljs@4.4.0` declara `interface Buffer extends ArrayBuffer {}` en su propio `.d.ts`; ese merge de tipo global no lo satisface ningún `Buffer` real de Node (`skipLibCheck` no alcanza porque el problema es el merge, no un error dentro del `.d.ts`), así que `workbook.xlsx.load(buffer)` necesita `buffer as any` — `unknown` no alcanza, ver decision log.
- `src/app/(app)/importar/page.tsx`, `actions.ts` e **`import-form.tsx`** — sube el archivo, muestra el conteo de filas leídas, las importadas y las saltadas por duplicado, y el detalle de las que fallaron. Es el único formulario de la app que necesita mostrar el resultado de una acción en la misma pantalla, así que es el primero en usar `"use client"` + `useActionState` — todos los formularios anteriores descartan el `Result` y dependen de `revalidatePath`.
- `scripts/parity-check.ts` — recalcula la suma directamente del `.xlsx` y la compara con la suma del libro **para la cuenta que el usuario eligió al importar** (por nombre, `"cuenta de ahorros"` — la misma convención que ya usan `commit.test.ts`/`receipts.test.ts`/etc. para "la" cuenta por defecto); imprime la diferencia y sale 1 si es distinta de 0. No suma indiscriminadamente cualquier cuenta con filas `source='excel_import'`: nada impide que otra parte de la app escriba esa fuente en otra cuenta (p. ej. un test de una fila que falla, deliberadamente en una cuenta aparte), y esas filas no le pertenecen al total de esta hoja.
- `tests/integration/import-excel.test.ts` — con `tests/fixtures/historical-sample.xlsx`: la importación escribe tres transacciones con `−$450.000`, `+$3.200.000` y `−$100.000`; importar el mismo archivo dos veces deja tres transacciones; una fila con monto no numérico se reporta como fallida sin abortar las demás. Ningún caso que vaya a confirmar usa contenido fijo (mismo motivo que el paso 12): el `source_ref` de este paso no tiene ni índice ni nombre de archivo, solo `fecha|descripcion|monto`, así que hasta un `descripcion` fijo colisiona entre corridas sueltas de `pnpm test` — usa un `descripcion` con `randomUUID()` para cualquier fila que se espera importar de verdad.
- `pnpm-workspace.yaml` — `overrides: { '@types/node': ^20 }`. `exceljs` trae `fast-csv` como dependencia, que a su vez declara `@types/node@14` como dependencia (no dev) — sin el override, dos versiones de `@types/node` conviven en el árbol y sus declaraciones globales de `Buffer` no siempre coinciden. Nota: **este override no es lo que arregla la incompatibilidad de `Buffer` de arriba** — la deduplicación es necesaria por higiene del árbol de todos modos, pero el cast `as any` sigue haciendo falta incluso con una sola versión de `@types/node`, porque la causa real es el merge del propio `.d.ts` de `exceljs`, no una duplicación de `@types/node`.
...
pkg.scripts = {
  ...
  build: "sh scripts/with-env.sh pnpm exec prisma generate && next build",
  start: "next start",
  ...
  postinstall: "sh scripts/with-env.sh pnpm exec prisma generate",
...
| **Decision: `commitPending` (paso 4) nombra el campo faltante dentro del mensaje `validation_failed`** | Dejar el mensaje genérico ("faltan campos para confirmar") que traía la primera versión del paso 4 | Why: descubierto al construir el paso 10 — el criterio de aceptación 1 de ese paso ("...whose message names the missing field...") ya estaba escrito así desde el principio en §9 paso 10 y el epic 02, pero `commitPending` nunca lo había cumplido porque el paso 4 nunca ejercitó ese criterio (pertenece al paso 10, no al 4). Bug latente desde el paso 4. La corrección arma un arreglo `missingFields` para el mensaje **y conserva la disyunción `||` original** sobre los mismos campos: TypeScript solo angosta `pending.accountId` etc. a no-nulos más abajo si la guarda es esa disyunción explícita, no `missingFields.length > 0`— quitar la disyunción rompe `tsc` con 5 errores de `string \| null`. Corregido en §9 paso 4 (Do) y el epic 01. Would reverse if: TypeScript aprende a angostar por el resultado de una función auxiliar en vez de por la expresión inline |
| **Decision: el `psql()` de los tests e2e lleva `-q` cuando captura un `INSERT ... RETURNING`** | Dejar `-t -A` sin `-q`, como en la primera versión de `revision.spec.ts` | Why: descubierto al correr el primer e2e del paso 10 — `psql -t -A` (sin `-q`) imprime el valor de `RETURNING` y *además* el tag de finalización (`INSERT 0 1`) en una segunda línea; el test capturaba ambas líneas como si fueran el id, y la siguiente consulta (`WHERE id = '<id>\nINSERT 0 1'`) fallaba con "invalid input syntax for type uuid". `-t -A` sola alcanza para un `SELECT` puro (usado sin problema en `ledger.spec.ts`), pero no para un `INSERT ... RETURNING`. Corregido en `tests/e2e/revision.spec.ts`, documentado como convención general en §3 y el epic 02, para que ningún spec e2e futuro que necesite el id de una fila recién insertada repita el bug. Would reverse if: se deja de usar `psql -c` para preparar estado de test y se pasa a un cliente Postgres real desde el spec |
| **Decision: `PendingTransaction` gana `sourceRef` (migración `add_pending_source_ref`), y `commitPending` lo propaga a `transactions.source_ref`** | Dejar `pending_transactions` sin esa columna, como especificaba §4 desde el principio | Why: descubierto al construir el paso 12 — el propio Do del paso ("Reprocesar el mismo extracto no duplica: la unicidad `(user_id, source, source_ref)` de §4 lo impide **al confirmar**") exige que el `source_ref` calculado por `statement-batch.ts` sobreviva hasta el momento de `commitPending`, pero `PendingTransaction` nunca tuvo esa columna y `commitPending` (paso 4) nunca mandaba `source_ref` al crear la `transaction` — sin este cambio, reprocesar un extracto ya parcialmente confirmado podía confirmar el mismo movimiento dos veces, exactamente lo que esa unicidad existe para impedir. `commitPending` también atrapa ahora la violación `P2002` de esa unicidad como `conflict` en vez de dejarla propagarse sin capturar — antes de este paso nada podía chocar contra ella porque nada mandaba `source_ref`. Migración aditiva (`ALTER TABLE ... ADD COLUMN`), sin downtime; `manual` y `receipt` siguen mandando `null`, que nunca colisiona. Corregido en §4 (schema), §9 paso 12 (Do) y el epic 02. Would reverse if: el confirmar-dos-veces-el-mismo-extracto deja de ser un riesgo real (p. ej. si `statement-batch.ts` se vuelve la única vía y ya filtra esto antes de crear el pendiente) |
| **Decision: `source_ref` de un movimiento de extracto incluye su índice dentro del archivo** | `source_ref = sha256(fecha\|descripcion\|monto\|nombreArchivo)`, tal como lo describe literalmente el Do del paso 12 | Why: descubierto al escribir `tests/integration/statements.test.ts` (paso 12) — el criterio de aceptación 4 exige conservar **ambos** cuando dos movimientos del mismo extracto son idénticos en fecha, descripción y monto; con la fórmula literal del Do, esos dos movimientos calculan el mismo `source_ref`, y aunque ambos pendientes se crean sin problema, confirmar el segundo choca contra la unicidad `(user_id, source, source_ref)` que el hallazgo anterior de esta misma tabla acaba de activar — el propio mecanismo que evita duplicar un extracto reprocesado bloquearía, por accidente, un extracto con dos compras iguales el mismo día. Se agrega el índice de posición dentro del arreglo que devuelve `extractStatement` como primer campo del hash. Sigue siendo idempotente entre corridas (misma posición, mismo archivo → mismo `source_ref`) mientras el modelo devuelva los movimientos en el mismo orden al reprocesar. Corregido en §9 paso 12 (Do) y el epic 02. Would reverse if: el orden de los movimientos que devuelve el modelo deja de ser estable entre llamadas |
| **Decision: `subir/actions.ts` rutea por `mimeType` — `application/pdf` a `processStatement`, imagen a `extractReceipt`** | Dejar que todo PDF pase por `extractReceipt` (paso 11), como sugería que ese paso ya aceptaba `application/pdf` entre los tipos de recibo | Why: descubierto al construir el paso 12 — ni el Do de este paso ni el de `subir` (paso 11) dicen cómo la interfaz distingue un recibo en PDF de un extracto en PDF, y el criterio de aceptación 5 de este paso exige que subir un extracto por la interfaz liste sus movimientos en `/revision`. Sin una segunda ruta, el único `/subir` del paso 11 seguiría mandando cualquier PDF a `extractReceipt`, que solo pide un movimiento — un extracto de varias filas perdería todas menos la que el modelo alucine como "la" transacción. La regla adoptada — PDF es siempre extracto, imagen es siempre recibo — dejó de contemplar un recibo en PDF a través de la interfaz (sigue siendo una entrada válida para quien llame `extractReceipt` directo); ningún acceptance de ningún paso pide lo contrario. Corregido en el epic 02 (E2-T5, Files). Would reverse if: aparece un flujo real de recibos en PDF y hace falta un control explícito en la interfaz para desambiguar |
| **Decision: `gateway.ts` gana un cliente de IA falso, activado solo bajo el mismo triple interlock que `e2eBypassUserId()`** | No darle a la suite e2e ninguna forma de sustituir el `AiClient`, y dejar `tests/e2e/subir.spec.ts` sin escribir o llamando a la API real | Why: descubierto al escribir el primer e2e que toca IA (paso 12 — ni el paso 6/7 de Telegram ni el paso 11 de recibos llegaron a necesitar uno). §13 es explícito: "Toda prueba inyecta un `AiClient` falso" — pero `tests/e2e/**` no puede importar `src/` (regla de §3) para pasarle uno al proceso del servidor real que levanta `pnpm test:e2e`. La única forma de cumplir ambas reglas a la vez es que el servidor mismo sepa cuándo está bajo e2e y sustituya el cliente ahí — mismo problema, misma solución, que `e2eBypassUserId()` ya resolvió para la sesión en el paso 3: activarlo requiere `E2E_USER_ID` presente, `DATABASE_URL` terminando en `_test`, y coincidiendo con `E2E_DATABASE_URL` — ninguna de las dos últimas llega nunca por un `.env`, así que esto no se puede activar por accidente contra dev o producción. Implementado en `extract-statement.ts`; cualquier extractor futuro que necesite un e2e propio sigue el mismo patrón en vez de tocar `gateway.ts`, que sigue siendo el único archivo que importa el SDK. Corregido en §9 paso 12 (Do, nota de §13) y el epic 02. Would reverse if: la suite e2e deja de correr contra un servidor real (p. ej. si se mueve a component testing con acceso directo a módulos de `src/`) |
| **Decision: todo contenido de `statements.test.ts` que un caso vaya a confirmar usa un `filename` único por corrida, nunca uno literal fijo** | Escribir cada `processStatement(...)` con `filename: "extracto.pdf"` fijo, como en el primer borrador del test | Why: descubierto corriendo `pnpm test` (suite completa) después de que `pnpm test tests/integration/statements.test.ts` (solo) ya había pasado en esta misma sesión — el test "keeps two movements... both confirmable" volvió a fallar con `pendingIds` en `0` en vez de `2`, porque el `source_ref` de sus dos movimientos (fecha, descripción y monto fijos, más el `filename` fijo) es determinístico y ya existía en `transactions` desde la corrida anterior: `statement-batch.ts` los saltó como "ya confirmados", el comportamiento correcto en producción pero no lo que este test quería medir. Mismo patrón que el error #8 de esta sesión (ids de test fijos chocan entre corridas) — nada trunca `transactions` entre invocaciones sueltas de `pnpm test`. Corregido en `tests/integration/statements.test.ts`: cada caso que puede llegar a confirmar usa `uniqueFilename()`. Would reverse if: la suite de integración empieza a truncar `transactions` antes de cada archivo (hoy no lo hace porque los archivos comparten conexión y `fileParallelism: false`) |
| **Decision: `pnpm-workspace.yaml` fija `overrides: { '@types/node': ^20 }`** | Dejar que `exceljs` (vía `fast-csv`, que declara `@types/node@14` como dependencia, no dev) siga conviviendo con el `@types/node@20` de la raíz | Why: descubierto al construir el paso 13 (primer uso real de `exceljs` para *leer* un archivo, no solo escribirlo como en `build-fixtures.ts`) — `pnpm why @types/node` mostraba dos versiones instaladas. El campo `pnpm.overrides` en `package.json` (donde vivía en versiones previas de pnpm) ya no lo lee `pnpm@11.24.0` — el ajuste se movió a `pnpm-workspace.yaml`, que este proyecto ya tenía por otras razones (`allowBuilds`, `minimumReleaseAgeExclude`). Corregido en §9 paso 13 (Do) y el epic 02. **Importante:** este override reduce el árbol a una sola versión de `@types/node`, pero **no** es lo que arregla el error de tipos de `Buffer` de la entrada siguiente — ese es un problema distinto, en el propio `.d.ts` de `exceljs`. Would reverse if: `fast-csv` deja de declarar `@types/node` como dependencia de producción, o `exceljs` deja de depender de `fast-csv` |
| **Decision: `workbook.xlsx.load(buffer)` necesita `buffer as any`, no `as unknown as Buffer`** | Usar un cast mínimo (`as unknown as Buffer`) para satisfacer el tipo, como en el primer intento | Why: descubierto al construir el paso 13 — `exceljs@4.4.0` declara `declare interface Buffer extends ArrayBuffer {}` en la primera línea de su propio `index.d.ts`. Esa es una declaración ambiente global que se fusiona con el `Buffer` real de `@types/node`; el `Buffer` resultante después de la fusión exige miembros de `ArrayBuffer` (`resizable`, `maxByteLength`, `resize`, `detached`, …) que ningún `Buffer` de Node expone jamás — no son parte de la clase, son parte de la interfaz `ArrayBuffer` con la que `exceljs` fusionó por error el nombre global `Buffer`. Deduplicar `@types/node` (entrada anterior) no lo arregla: el conflicto no es de versiones, es un merge de tipos malformado dentro del propio paquete. `skipLibCheck` (ya activado en `tsconfig.json`) tampoco alcanza, porque no es un error *dentro* del `.d.ts` de `exceljs`, es una declaración que se fusiona correctamente y produce un tipo global imposible de satisfacer en cualquier archivo que lo use — `skipLibCheck` solo deja de *chequear* el archivo de tipos en sí. `as unknown as Buffer` falla porque el cast resuelve al mismo `Buffer` ya fusionado y roto: la expresión sigue sin satisfacerlo. Solo `as any` escapa la comprobación por completo. La conversión es segura en runtime: `exceljs` solo usa el buffer como bytes, nunca invoca ninguno de esos miembros de `ArrayBuffer`. Corregido en §9 paso 13 (Do), `import-excel.ts` y `parity-check.ts`. Would reverse if: `exceljs` publica una versión que corrige o retira esa declaración |
| **Decision: `parity-check.ts` compara la cuenta `"cuenta de ahorros"` por nombre, no cualquier cuenta con filas `source='excel_import'`** | Agrupar por `accountId` y comparar cada cuenta encontrada contra el total del archivo, como en el primer borrador del script | Why: descubierto ejecutando `parity-check.ts` después de `tests/integration/import-excel.test.ts` — el propio test del paso 13 (el caso de "fila con monto inválido") escribe, a propósito, una fila `excel_import` legítima en `cuenta corriente` para no ensuciar la cuenta del historico canónico; con el script original, esa fila hacía que `parity-check.ts` comparara TAMBIÉN `cuenta corriente` contra el total completo del `.xlsx` — un total que nunca le correspondió — y fallaba con una diferencia de cientos de miles de pesos. El script no tiene forma de saber, solo mirando `transactions`, cuál cuenta es "la" del historico — no hay ninguna columna que lo marque. Se resuelve apuntando al nombre `"cuenta de ahorros"`, la misma convención que ya usan `commit.test.ts`, `receipts.test.ts`, `review.test.ts` y este mismo paso para "la" cuenta por defecto en pruebas. Corregido en §9 paso 13 (Do) y el epic 02. Would reverse if: la importación deja de ser de una sola cuenta, o se agrega una forma explícita de marcar la cuenta del historico |
| **Decision: el gate global de §20.1 corre `pnpm db:migrate` + `pnpm db:seed` y `sh scripts/backup.sh` antes de `sh scripts/restore-check.sh`** | Dejar la lista tal como la traía §20.1 desde antes de este paso: `... pnpm test:e2e`, `sh scripts/restore-check.sh`, conteo de tags | Why: descubierto construyendo el paso 14, releyendo con cuidado qué ejecutaría `ci.yml` de verdad en un runner limpio, no solo transcribiendo la lista existente — `restore-check.sh` restaura el dump **más reciente** de `backups/`, un directorio en `.gitignore` que ningún runner de CI empieza teniendo; sin un `sh scripts/backup.sh` en algún paso previo de la misma corrida, `restore-check.sh` sale 1 de inmediato con "no hay ningún respaldo", y el gate global — que existe precisamente para que un build nunca avance sin un respaldo restaurable probado — fallaría siempre, en cada corrida, sin ninguna forma de pasar. Además, sin `pnpm db:migrate` (contra `DATABASE_URL`, no `TEST_DATABASE_URL` — esa la corre `db:migrate:test`, ya presente) antes de `backup.sh`, la base `personal_finance` de un runner limpio nunca recibe ninguna migración: el `pg_dump` sale bien igual (una base vacía es un dump válido), pero el conteo de tablas que compara `restore-check.sh` pasa trivialmente sin comparar nada real — un "pase" hueco que no prueba lo que el gate dice probar. Ninguno de los `grep` del criterio 5 original habría atrapado esto: comprueban que cada comando *aparezca* en `ci.yml`, no que la app realmente funcione al ejecutarse; ambos problemas solo aparecen razonando sobre la ejecución real, no sobre el texto. Corregido en §20.1, §9 paso 14 (Do, Done when, Verify — con dos `grep` nuevos, `pnpm db:migrate`/`pnpm db:seed` anclados con `[[:space:]]*$` para no matchear sus variantes `:test`), el epic 02, y `.claude/settings.json` (`pnpm db:migrate`/`pnpm db:seed` sin `:test` entran al allowlist, que §19.3 promete cubrir todo comando del gate). Would reverse if: `restore-check.sh` deja de depender de un archivo en `backups/` (p. ej. si se le pasa la ruta del dump como argumento y el gate lo genera por otro medio) |
| **Decision: `.github/workflows/ci.yml` empieza con `cp .env.example .env` y levanta Postgres con el `docker-compose.yml` del propio repo, no el bloque `services:` nativo de GitHub Actions** | Declarar `services: { postgres: {...} }` en el YAML del workflow, como es más común en ejemplos de GitHub Actions | Why: decidido al escribir el paso 14 — un runner de GitHub Actions nunca tiene `.env` (está en `.gitignore` a propósito, nunca se commitea) y todos los scripts de `package.json` que usan Prisma/tsx/Vitest pasan por `scripts/with-env.sh`/`with-test-env.sh`, que esperan uno; `.env.example` ya trae valores que funcionan contra Postgres local sin ninguna credencial real (los tests inyectan un `AiClient` falso, el e2e usa el bypass de auth del paso 3, `STORAGE_DRIVER=local` evita Supabase Storage), así que copiarlo es suficiente y no inventa un segundo archivo de configuración para CI. Usar el `docker-compose.yml` del repo (en vez de `services:` de Actions) mantiene un solo lugar de verdad para cómo se levanta Postgres en este proyecto, y es exactamente el mismo comando (`docker compose up -d --wait`) que corre en la máquina de Alejandro — nada que mantener sincronizado entre dos formatos. Corregido en §9 paso 14 (Do) y el epic 02. Would reverse if: el proyecto necesita un servicio distinto de Postgres para pruebas paralelas que `docker-compose.yml` no modela |
| **Decision: el script `typecheck` corre `next typegen && tsc --noEmit`, no `tsc --noEmit` solo** | Dejar `typecheck: "tsc --noEmit"` como lo escribió el Bootstrap del paso 1 | Why: **descubierto por la primera corrida real de `ci.yml` en GitHub Actions**, después de empujar el repo — `tsc --noEmit` falló en un checkout limpio con `error TS2304: Cannot find name 'LayoutProps'` en `src/app/layout.tsx`. `LayoutProps<"/">` no es un tipo de Next.js, es un tipo **generado**: Next.js lo escribe en `.next/types/routes.d.ts` recien la primera vez que corre `next dev` o `next build`, y `tsconfig.json` (el que emite `create-next-app`, sin editar en ningún paso) incluye `.next/types/**/*.ts` en su `include`. `.next/` está en `.gitignore` por diseño — nunca viaja al repo — así que en la máquina de esta sesión el bug estuvo invisible desde el paso 1: `pnpm dev`/`pnpm build` corrieron decenas de veces a lo largo de las catorce tareas, y esa carpeta nunca se borró entre medio, así que cada `pnpm typecheck` posterior encontraba el tipo ya generado por casualidad. Un checkout nuevo — cualquier CI, cualquier clon fresco, o simplemente borrar `.next/` — nunca lo tiene, y `pnpm typecheck` es literalmente el primer comando del gate global, antes de `pnpm build`. `next typegen` (Next.js 16) genera esos mismos tipos sin build completo, así que anteponerlo es barato y no requiere ningún env var. Corregido en `package.json`, §10 Bootstrap (el `.bootstrap-patch.mjs` que escribe `scripts.typecheck`) — retroactivo a **todo** paso de §9 que corre `pnpm typecheck` como parte de su `Verify`, no solo el 14. Would reverse if: Next.js deja de requerir generación de tipos para rutas tipadas, o el proyecto desactiva esa característica |
| **Decision: `.env.example` trae `TELEGRAM_BOT_TOKEN='local-placeholder-bot-token'` y `TELEGRAM_ALLOWED_CHAT_ID='123456789'`, no cadenas vacías** | Dejar ambas en `''`, como las escribió el paso 10 del Bootstrap (§10) | Why: **descubierto por la segunda corrida real de `ci.yml`**, después de corregir el hallazgo anterior — `pnpm test` falló las 4 pruebas de `telegram-webhook.test.ts` con `requireTelegram: variables de entorno invalidas o ausentes: TELEGRAM_BOT_TOKEN, TELEGRAM_ALLOWED_CHAT_ID`. La máquina de esta sesión tenía un `.env` local con valores placeholder no vacíos para esas dos variables — escritos en algún punto del paso 6/7 para poder probar el webhook localmente — pero esa corrección nunca se propagó a `.env.example`, el archivo que sí se commitea y que §10 promete mantener sincronizado ("`.env.example` esta commiteado y se mantiene sincronizado"). `ci.yml` arranca con `cp .env.example .env` (paso 14) contra un runner que nunca tuvo un `.env` local con el ajuste; con la plantilla vacía, `requireTelegram()` lanza apenas el webhook la invoca, aunque el test inyecta un canal falso — el fallo es de configuración, no de lógica. Confirmado reproduciendo el fallo local antes de arreglarlo: `cp .env.example .env` seguido de `pnpm test` reproduce exactamente el mismo error que reportó CI, y vuelve a pasar con el `.env.example` corregido. Corregido en `.env.example` (el del repo) y `workspace/.env.example` (la plantilla que el Bootstrap copia — de lo contrario una ejecución fiel futura del blueprint reintroduce el mismo bug). Would reverse if: `requireTelegram()` deja de exigir estas dos variables, o el webhook de Telegram deja de tener cobertura de integración |
| **Decision: `package.json` gana el script `postinstall: "sh scripts/with-env.sh pnpm exec prisma generate"`** | Dejar `db:generate` como el único comando que genera el cliente de Prisma, corrido a mano en cada build local | Why: **descubierto por el primer deploy real en Vercel**, después de que el gate local y el de CI ya estaban en verde — el build falló con `Module not found` resolviendo `../../generated/prisma` desde `src/server/db/client.ts`. `src/generated/prisma` está en `.gitignore` (es código generado) y ningún paso de §9 ni de `ci.yml` lo regenera salvo `pnpm db:generate`, que nadie invoca automáticamente antes de `next build` en Vercel — Vercel solo corre `pnpm install` y el comando de build que declara el proyecto. Localmente el bug estuvo invisible toda la sesión porque `pnpm db:generate` se corrió a mano decenas de veces desde el paso 2. `ci.yml` tampoco lo habría atrapado: su paso `Generate Prisma client` (`pnpm db:generate`) SÍ está explícito ahí, a diferencia del pipeline de build de Vercel, que no comparte esa lista. Un script `postinstall` es el mecanismo estándar que los hosts de Node (Vercel incluido) ya invocan automáticamente después de `pnpm install`, sin necesitar configuración adicional del lado de Vercel. Corregido en `package.json` y §10 Bootstrap (el `.bootstrap-patch.mjs` que escribe `scripts`). Would reverse if: el proyecto deja de generar un cliente de Prisma fuera de `node_modules`, o Vercel dejara de ejecutar hooks de `postinstall` |
| **Decision: `package.json` también antepone `prisma generate` al propio `build` (`build: "sh scripts/with-env.sh pnpm exec prisma generate && next build"`), no solo a `postinstall`** | Confiar únicamente en el `postinstall` de la decisión anterior | Why: **descubierto por un deploy real posterior en Vercel** — con `postinstall` ya en su lugar y el gate en verde, un push directo a `main` volvió a fallar con el mismo `Module not found` en `../../generated/prisma`, mientras que varios "Redeploy" manuales del mismo commit sí pasaron. El log del build que falló mostraba `Restored build cache from previous deployment` seguido de `Already up to date` en el paso de instalación: cuando Vercel restaura la caché de `pnpm` y el lockfile no cambió, pnpm considera la instalación ya satisfecha y **no vuelve a correr `postinstall`** — pero cada deploy es un clon nuevo del repo, así que `src/generated/prisma` (gitignored, nunca viaja en la caché de dependencias) no existe en ese árbol de trabajo. Los "Redeploy" manuales solo pasaban porque se lanzaron con "Use existing Build Cache" desmarcado, forzando una instalación completa que sí corre `postinstall`; un push normal a `main` usa caché por defecto. Encadenar `prisma generate` directo al comando `build` lo hace incondicional: corre en cada build sin importar si pnpm decidió saltarse los scripts de instalación. Confirmado leyendo el log de build de Vercel línea por línea (`Restored build cache...` → `Already up to date` → `Module not found`). Corregido en `package.json` y §10 Bootstrap (el `.bootstrap-patch.mjs` que escribe `scripts.build`); `postinstall` se deja igual, es inofensivo y sigue ayudando a una instalación local limpia sin caché de Vercel de por medio. Would reverse if: Vercel deja de restaurar caché de `pnpm` entre deploys, o pnpm deja de saltarse `postinstall` en un cache hit |
| **Decision: rediseño visual completo del libro (`/`) — paleta cálida, Playfair Display/Source Sans 3 vía `next/font/google`, tabs Hoy/Historial/Resumen, FAB + bottom-sheet, chips en vez de `<select>`** | Mantener la paleta azul/system-font y la tabla densa siempre visible del diseño original de §7 | Why: **pedido explícito del usuario post-deploy** ("yo pensaba algo más así"), mostrando un mockup propio (`expense-tracker.jsx`, artifact de otra conversación de Claude — localStorage, sin cuentas ni ingresos, categorías fijas). Se le presentaron tres alcances (rediseño completo / híbrido de patrones sin paleta / solo estructura) vía pregunta directa porque el mockup contradecía decisiones ya deliberadas y documentadas en §7 (primary azul, "sin webfont", elevación plana, tabla densa) — no una que un agente debía resolver solo. Eligió rediseño completo. El monto (`amount_cents`, bigint) ahora cruza la frontera Server→Client Component como `amountCentsStr` (string) porque React Flight no serializa `bigint` en props — se reconvierte a `bigint` justo antes de llamar a `src/lib/money.ts`, así que la regla de "el dinero nunca toca un float" se mantiene intacta, solo cambia el transporte. El límite de 50 filas del libro original se eliminó: Historial necesita el histórico completo para agrupar por mes, y a escala de un solo usuario cargarlo entero es aceptable. El FAB abre un bottom-sheet con foco automático en "Descripción" y cierre por Escape — sin esto el usuario de teclado queda con el foco fuera del contenido recién aparecido. Confirmado con `pnpm build` local y con el deploy real en Vercel (mismo flujo que las decisiones anteriores de esta sección — no se pudo iniciar sesión localmente sin la contraseña real de Supabase). `tests/e2e/ledger.spec.ts` se reescribió por completo: roles `listitem` en vez de `row`/`tbody tr`, flujo FAB→dialog para el alta, borrado de dos toques, y el test de teclado ahora abre el modal con Enter y verifica el foco automático en Descripción. Corregido en `src/app/globals.css` (tokens), `src/app/layout.tsx` (fuentes), `src/app/(app)/layout.tsx` + `nav-links.tsx` (nav con pill activo), `src/app/(app)/page.tsx` + `ledger-view.tsx` + `history-view.tsx` + `summary-view.tsx` + `transaction-row.tsx` + `transaction-form-modal.tsx` + `ledger-types.ts` + `empty-state.tsx` (el libro completo), `src/lib/category-color.ts` (color determinístico por nombre de categoría, nuevo), encabezados de `cuentas/`, `revision/`, `subir/`, `importar/`, `login/` (tipografía), `CLAUDE.md` §Design system, y `tests/e2e/ledger.spec.ts`. Would reverse if: el usuario pide volver al diseño original, o `next/font/google` deja de poder autohostear en build time |
| **Decision: `next.config.ts` gana `allowedDevOrigins: ["127.0.0.1"]`** | Dejar la config de Next.js sin ese campo, como la trae `create-next-app` por defecto | Why: **descubierto por `ci.yml` fallando en las 4 pruebas nuevas de `tests/e2e/ledger.spec.ts` que dependen de interactividad de cliente (FAB, tabs, chips)**, mientras que las mismas pruebas contra el deploy real de Vercel funcionaban perfecto — eso descartó un bug de producto de entrada. El log de build del `webServer` de Playwright mostraba, en cada corrida, `Blocked cross-origin request to Next.js dev resource ... from "127.0.0.1"` para los chunks de JS y `/_next/hmr`. `playwright.config.ts` define `baseURL` como `http://127.0.0.1:<puerto>` (no `localhost`), y la protección de origen cruzado que Next.js activa por defecto en dev no incluye `127.0.0.1` en su lista permitida — el navegador de Playwright pedía los chunks del cliente y el propio dev server los rechazaba, así que ningún componente cliente hidrataba nunca y cualquier `onClick` quedaba muerto, sin ningún error visible ni en la consola del test ni en el log de Playwright (el click en si "funciona": encuentra el boton, lo hace foco, dispara el evento — solo que nadie escucha). Esto estuvo invisible en toda prueba e2e anterior a este rediseño porque cada una dependía solo de `<form action={...}>` (Server Actions con fallback de POST nativo, funciona sin JS) — el libro es la primera pantalla de la app cuya interacción probada por e2e no tiene ningún camino sin JavaScript. Confirmado leyendo `test-results/*/error-context.md` de un run real de CI (subido como artifact solo para este diagnóstico, ver decision siguiente): el snapshot de accesibilidad tras el click muestra el FAB todavía presente y ningún `dialog`, es decir cero cambio de estado. Corregido en `next.config.ts`. Would reverse if: Playwright empieza a navegar por `localhost` en vez de `127.0.0.1`, o Next.js cambia el default de `allowedDevOrigins` |
| **Decision: `ci.yml` sube `test-results/` como artifact (`playwright-traces`) cuando `E2E tests` falla** | Dejar que el trace/`error-context.md` que Playwright ya escribe en disco (`trace: "retain-on-failure"` en `playwright.config.ts`) se pierda con el runner | Why: sin esto, diagnosticar el hallazgo anterior habría sido puro ensayo-y-error contra minutos de CI del usuario — el log de texto de `pnpm test:e2e` no incluye el snapshot de accesibilidad ni el trace visual que sí trae `error-context.md`. Es una mejora permanente, no solo para este incidente: cualquier fallo futuro de e2e en CI ahora es diagnosticable sin reproducirlo a mano. Corregido en `.github/workflows/ci.yml` (paso nuevo, `if: failure()`, `actions/upload-artifact@v4`, `retention-days: 7`). Would reverse if: el proyecto adopta un reporter de Playwright que ya publique esto en otro lado |
| **Decision: los lookups por `psql` de cuenta/categoría en `tests/e2e/ledger.spec.ts` filtran `and archived_at is null`** | Consultar solo por `user_id`, como en el resto de los `select` de la suite | Why: **descubierto leyendo el segundo trace de CI tras el fix de `allowedDevOrigins`** — el bottom-sheet ya abría, pero `fillManualEntry` se colgaba 30s esperando un chip de cuenta llamado `cuenta auditada <uuid> renombrada`. Esa fila es exactamente lo que deja `cuentas.spec.ts` (`creates, renames and archives an account`), que corre antes en la misma suite; `accounts` nunca se trunca entre specs (a diferencia de `transactions`/`pending_transactions`, que sí trunca el `beforeEach` de este archivo), así que la cuenta archivada sobrevive para el resto de la corrida. El `select` original no filtraba `archived_at`, pero el query real del libro sí (`archivedAt: null`, igual que antes en el `<select>` que reemplazó) — la UI correctamente nunca muestra esa cuenta como chip, y el test esperaba para siempre un botón que no iba a aparecer. Confirmado en el mismo trace: los otros tres fallos de `ledger.spec.ts` de esa corrida eran el mismo síntoma exacto. Corregido en las cuatro consultas de cuenta/categoría de `tests/e2e/ledger.spec.ts`. Would reverse if: `cuentas.spec.ts` deja de archivar la cuenta que crea, o la suite empieza a truncar `accounts` entre specs |
| **Decision: `tests/e2e/subir.spec.ts` espera `networkidle` entre el click en "Subir" y el `goto("/revision")`** | Navegar a `/revision` inmediatamente después de hacer click, como ya lo hacía este archivo desde antes de esta sesión | Why: **carrera preexistente, no causada por el rediseño**, expuesta cuando el resto de la suite empezó a correr más rápido (los fixes anteriores de `ledger.spec.ts` le sacaron ~30s de timeouts muertos a la corrida total). `uploadReceiptAction` corre la extracción de forma síncrona en el server action, pero un click sobre `<form action={...}>` no espera esa respuesta antes de devolver el control — el `goto` siguiente podía llegar a `/revision` antes de que el `pending_transaction` estuviera escrito, y el test veía "No hay pendientes por revisar." con el `pending_transactions`/`transactions` recién truncados por el `beforeEach` de `ledger.spec.ts` (spec anterior en el orden de archivos) todavía frescos. Confirmado con dos corridas consecutivas fallando de forma identica e idéntica (no intermitente) una vez que el resto de la suite dejó de tener los timeouts de 30s que enmascaraban el timing real. Corregido en `tests/e2e/subir.spec.ts`. Would reverse if: `uploadReceiptAction` deja de ser síncrono, o el formulario deja de depender de una navegación posterior para verse |
